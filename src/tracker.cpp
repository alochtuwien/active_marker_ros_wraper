//
// Created by aloch on 24.01.23.
//

#include "../include/tracker.hpp"

TrackedBlob::TrackedBlob(cv::Point2d &center, double radius) {
...
void TrackedBlob::cluster_analytics() {
    ...
    radius = std::min(max_roi_radius, ((1. - gamma) * radius + (3 * gamma * avg_distance)));
}

TrackerOut TrackedBlob::get_output() {
    return TrackerOut{.x = center_position.x, .y = center_position.y, .r = radius};
}
